...
from hosted_facilitator.nanopayments.constants import (
    ...
    GATEWAY_X402_TRANSFER_PATH_TEMPLATE,
...
def _convert_requirements_for_circle(req_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    Our PaymentRequirementsKind.to_dict() produces:
        ...
        - extra: Circle Gateway metadata from supported kinds
    ...
    for kind in accepts:
        ...
        converted_kind: dict[str, Any] = {
            ...
            "maxTimeoutSeconds": kind.get("maxTimeoutSeconds", MAX_TIMEOUT_SECONDS),
        }
...
class NanopaymentHTTPClient:
    ...
    Handles:
    - Public x402 Gateway endpoint calls
    - Timeout enforcement
...
class NanopaymentClient:
    ...
    Args:
        ...
        api_key: Deprecated and ignored. Circle Gateway x402 supported,
            verify, and settle endpoints do not require a Circle API key.
    ...
    async def verify(
        ...
        return VerifyResponse(
            ...
            invalid_reason=(
                ...
                else "malformed_gateway_verify_response"
            ),
        )
    ...
    async def settle(
        ...
        if success is not True:
            if success is not False:
                raise GatewayAPIError(
                    message="Gateway /settle returned malformed success field",
                    status_code=502,
                    response_body=resp.text,
                )
        ...
        if not isinstance(transaction, str) or not transaction.strip():
            raise GatewayAPIError(
                message="Gateway /settle returned success without transaction",
                status_code=502,
                response_body=resp.text,
            )
    ...
    async def get_x402_transfer(self, transfer_id: str) -> dict[str, Any]:
        """
        Retrieve a Circle Gateway x402 transfer by ID for reconciliation.

        The settle response transaction field is the Gateway transfer UUID. This
        endpoint returns its lifecycle status, including received, batched,
        ...
        if not transfer_id:
            raise GatewayAPIError(
                message="Gateway transfer ID is empty",
                status_code=400,
                response_body=None,
            )
        path = GATEWAY_X402_TRANSFER_PATH_TEMPLATE.format(id=quote(transfer_id, safe=""))
        async with NanopaymentHTTPClient(
            base_url=self._base_url,
            timeout=self._timeout,
        ) as http:
        ...
        if not isinstance(data, dict):
            raise GatewayAPIError(
                message="Gateway transfer lookup returned malformed body",
                status_code=502,
                response_body=resp.text,
            )
        return data
