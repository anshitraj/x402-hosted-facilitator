...
class PaymentRequirementsExtra:
    ...
    """Minimum signature validity required by Gateway, when advertised."""

    assets: tuple[dict[str, Any], ...] = ()
    """Gateway-supported assets exactly as returned in extra.assets."""
    ...
    def from_dict(cls, data: dict[str, Any]) -> PaymentRequirementsExtra:
        ...
        if isinstance(assets, list | tuple):
            parsed_assets = tuple(dict(asset) for asset in assets if isinstance(asset, dict))
        else:
            parsed_assets = ()
        min_validity = data.get("minValiditySeconds")
        if min_validity is not None:
            try:
                min_validity = int(min_validity)
        ...
        return cls(
            ...
            min_validity_seconds=min_validity,
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.min_validity_seconds is not None:
            data["minValiditySeconds"] = self.min_validity_seconds
        if self.assets:
            data["assets"] = [dict(asset) for asset in self.assets]
        return data
...
class PaymentRequirementsKind:
    ...
    Circle Gateway's GatewayEvmScheme currently uses 604900 seconds.
    """
...
class SupportedKind:
    ...
    - assets: supported assets, including USDC address and decimals
    - minValiditySeconds: minimum validity window for signed authorizations
    """
    ...
    def usdc_address(self) -> str | None:
        ...
        if not self.extra:
            return None
        assets = self.extra.get("assets")
        if isinstance(assets, list | tuple):
            for asset in assets:
                if not isinstance(asset, dict):
                    continue
                if asset.get("symbol") == "USDC" and isinstance(asset.get("address"), str):
                    return asset["address"]
        if isinstance(self.extra.get("usdcAddress"), str):
            return self.extra["usdcAddress"]
        return None
...
class NanopaymentResult:
    """Result of a Gateway nanopayment execution."""
