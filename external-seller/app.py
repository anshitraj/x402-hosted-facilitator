...
def _seller_pay_to() -> str:
    explicit = _env("SELLER_PAY_TO") or _env("X402_SELLER_PAY_TO")
    if explicit:
        return Web3.to_checksum_address(explicit)
    raise RuntimeError("Set SELLER_PAY_TO in external-seller/seller.env")


def _facilitator_endpoint() -> str:
    configured = (
        _env("SELLER_FACILITATOR_URL")
        or _env("X402_FACILITATOR_URL")
        or "http://127.0.0.1:4022/v2/x402"
    ).rstrip("/")
    if configured.endswith("/v2/x402"):
        return configured
    return f"{configured}/v2/x402"


def _api_key() -> str:
    return _env("SELLER_API_KEY") or _env("X402_FACILITATOR_API_KEY")
...
def _facilitator_client() -> HTTPFacilitatorClient:
    ...
    if not headers:
        return HTTPFacilitatorClient(FacilitatorConfig(url=FACILITATOR_ENDPOINT))

    def create_headers() -> dict[str, dict[str, str]]:
        return {"supported": headers, "verify": headers, "settle": headers}

    return HTTPFacilitatorClient({"url": FACILITATOR_ENDPOINT, "create_headers": create_headers})


def _supported_kinds() -> list[dict[str, Any]]:
    response = httpx.get(f"{FACILITATOR_ENDPOINT}/supported", headers=_auth_headers(), timeout=15.0)
...
def _arc_usdc_amount(decimal_amount: float | str | Decimal, network: str) -> AssetAmount | None:
    ...
    return AssetAmount(amount=str(int(scaled)), asset=asset, extra={"name": "USDC", "version": "2"})


def _accept_mode() -> str:
    value = (_env("SELLER_ACCEPT_MODE") or _env("X402_ACCEPT_MODE") or "all").lower()
    if value not in {"all", "exact", "gateway"}:
        raise RuntimeError("SELLER_ACCEPT_MODE must be all, exact, or gateway")
    return value
...
def _payment_options() -> list[PaymentOption]:
    ...
    for kind in SUPPORTED_KINDS:
        if kind.get("scheme") != "exact" or kind.get("network") != SELLER_NETWORK:
            continue
    ...
    if not options:
        raise RuntimeError("No usable x402 rails were returned by the hosted facilitator")
    return options
...
def _payment_option_summary(option: PaymentOption) -> dict[str, Any]:
    ...
    return {
        ...
        "gatewayBatch": extra.get("name") == "GatewayWalletBatched",
    }
...
FACILITATOR_ENDPOINT = _facilitator_endpoint()
SELLER_NETWORK: Network = _env("SELLER_NETWORK", ARC_TESTNET_NETWORK)
PRICE = _env("SELLER_PRICE", "$0.001")
PAY_TO = _seller_pay_to()
SUPPORTED_KINDS = _supported_kinds()
PAYMENT_OPTIONS = _payment_options()

app = FastAPI(title="External x402 Seller")
...
routes: dict[str, RouteConfig] = {
    "GET /compute": RouteConfig(
        ...
        description="External seller compute endpoint using the OmniClaw hosted facilitator",
    )
}
...
async def health() -> dict[str, Any]:
    return {
        "status": "ok",
        "service": "external-x402-seller",
        "facilitator": FACILITATOR_ENDPOINT,
        "network": SELLER_NETWORK,
        "price": PRICE,
        "payTo": PAY_TO,
    ...
    return JSONResponse(
        {
            "service": "external-x402-seller",
            "network": SELLER_NETWORK,
            "asset": _asset_for_network(SELLER_NETWORK),
            "price": PRICE,
            "payTo": PAY_TO,
            "input": {"size": size},
